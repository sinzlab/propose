...
def test_get_edge_index():
    ...
    data = HeteroData({
        'x': dict(x=x),
        ('x', '->', 'x'): dict(edge_index=edge_index)
    })
...
def test_forward(cond_gcn_mock, module_list_mock):
    ...
    cond_gcn_mock.return_value = (dict(x=x_return_value), dict(x=2))  # x_dict, edge_index_dict
    ...
    assert cond_gcn_mock.mock_calls[0] == call(in_features, context_features, hidden_features, hidden_features)
    assert cond_gcn_mock.mock_calls[1] == call(hidden_features, hidden_features, out_features, hidden_features)
