...
class FastCondGCN(nn.Module):
    ...
    def message(self, embeds: dict, edge_index_dict: dict) -> tuple[torch.Tensor, torch.Tensor]:
        keys = {
            'x': ['x', 'x->x', 'x<-x'],
            'c': ['c', 'c->x']
        }
        for key in edge_index_dict.keys():
            ...
            output_select = (idx * self.features['hidden'], (idx + 1) * self.features['hidden'])
    ...
    def aggregate(self, message: tuple[torch.Tensor, torch.Tensor], self_x: torch.Tensor) -> torch.Tensor:
        ...
        value = torch.cat([
            *values,  # concatenate all the message values
            self_x
        ])
